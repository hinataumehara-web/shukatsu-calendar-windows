...
押すだけです。ファイルの編集も Google アカウントの設定も要りません（15分ほど）。
できた `shukatsu.ics` を自分のカレンダーに取り込めば、スマホにも通知が届きます。

以下は、仕組みまで含めて把握したい人向けの詳しい説明です。

### Google でログイン方式を用意するには

**未実施。現状このリポジトリに `oauth_client.json` は入っていない。**
やろうとして要件に突き当たったので、分かったことを残しておく。

「Google でログイン」を配るには、OAuth 同意画面を**「本番」に公開**する必要がある。
「テスト中」のままだとリフレッシュトークンが**7日で失効**し、
受け取った人のツールがある日静かに止まるため、テスト状態での配布は現実的でない。

そして本番公開には、Google の要件として次が必要になる。

| 必要なもの | 内容 |
|---|---|
| ホームページ URL | プライバシーポリシーへのリンクを置いたページ |
| プライバシーポリシー URL | スマホ表示に対応していること |
| 利用規約 URL | |
| 承認済みドメイン | 上記 URL のドメイン。**Search Console での所有権確認が必要** |

つまり「クライアントを作って同梱するだけ」では済まず、**サイトを1つ用意して
ドメインの所有権を証明する**作業が要る。個人ツールを友人数人に配る目的に対しては
重いので、このリポジトリでは **A（ICS ファイル方式）を既定**にしている。

将来やるなら、GitHub Pages に3ページを置いて Search Console で所有権確認をし、
`hinataumehara-web.github.io` を承認済みドメインに登録する、という道筋になる
（`github.io` が承認済みドメインとして受け付けられるかは未検証）。

公開できた場合の残りの手順は次のとおり。

1. 「クライアント」→「クライアントを作成」→ 種類は **デスクトップ アプリ**
2. JSON をダウンロードし、`oauth_client.json` としてリポジトリ直下に置いて push
   （雛形は `oauth_client.example.json`）
3. スコープは `https://www.googleapis.com/auth/calendar.events` の**1つだけ**にする
   （`.events` の付かない `calendar` は権限が広すぎる）

> **利用者は100人まで**。審査を通していないアプリの上限で、友人数人なら問題にならない。

> 受け取る側には初回に「**このアプリは確認されていません**」と表示される。
> 何も知らずに見ると進めない見た目なので、事前に伝えておくこと。

> **client_secret をリポジトリに置くことについて**: デスクトップアプリの
> シークレットは Google 自身も機密として扱わない前提だが、公開リポジトリに
> 置けば第三者が使える。悪用されたらコンソールから無効化して作り直す。
...
| タブ | できること |
|---|---|
| 1. カレンダー | Google の鍵ファイルの取り込み、**共有先アドレスのコピー**、カレンダー ID、**卒業予定年**、通知日数、**接続テスト** |
...
締切をカレンダーに入れる方法は3つある。**既定は A。人に配るときも A。**

| | A. ICS ファイル | B. Google でログイン | C. サービスアカウント |
|---|---|---|---|
| 受け取る側の準備 | **なし** | ログイン1回 | Google Cloud で20分 |
| **配る側の準備** | **なし** | **サイトの公開が必要**（下記） | なし |
| 毎日の手間 | 取り込み操作 数クリック | なし（全自動） | なし（全自動） |
| スマホ通知 | 届く | 届く | 届く |
| 追加インストール | なし | `setup_google.bat` | `setup_google.bat` |
| 対応カレンダー | Google / Outlook / Apple | Google のみ | Google のみ |

**B はコードとしては動くが、このリポジトリでは現在使えない。** 配布に必要な
`oauth_client.json` を同梱していないためで、用意するには Google の要件
（ホームページ・プライバシーポリシー・利用規約の公開とドメイン所有権の確認）を
満たす必要がある（→「[Google でログイン方式を用意するには](#google-でログイン方式を用意するには)」）。
...
#### B. Google でログイン（配る側の準備が要る）

**配布物に `oauth_client.json` が含まれている場合だけ使える。** 含まれていれば、
設定画面の **「Google でログイン」** を押すだけ。ブラウザが開くのでアカウントを選び、
カレンダーへのアクセスを許可すれば完了する。以後は自動で自分の
Google カレンダーに登録され、スマホにも通知が届く。

同梱されていない場合、設定画面には「Google への接続手段がありません」と出る。
その場合は A（ICS ファイル）を使う。

```bat
setup_gui.bat            :: 「1. カレンダー」タブ →「Google でログイン」
run.bat --connect-google :: 同じこと（コマンドから）
```

カレンダー ID は空のままでよい（自分のメインカレンダーに入る）。

> **初回に警告画面が出る**: 「このアプリは確認されていません」と表示される。
> Google の審査を通していない個人ツールでは避けられない。配る側が
> 事前に「自分が作ったツールだ」と伝えておくこと。
> 「詳細」→「（アプリ名）に移動」で進める。

> 求める権限は**予定の読み書きだけ**（`calendar.events`）。カレンダーの
> 作成・削除やメールなど、他のデータには触れない。

#### C. サービスアカウント（上級者向け）

配る人自身が完全無人で回したい場合や、専用カレンダーに分けたい場合。
まず `setup_google.bat` を実行してライブラリを入れ、次の設定をする。
...
| `mynavi` | マイナビ | 手動 | ログイン導線とマイページ URL は確認済み。**一覧ページの締切パターンは未調整**（`enabled: false`） |
...
まず**卒業予定年**を設定する。マイナビは URL に卒業年が入っていて
（2028年卒なら `https://job.mynavi.jp/28/...`）、ここが合っていないと
別の年度のマイページを見にいってしまう。設定画面の「卒業予定年」か、
`config.yaml` の `settings.grad_year` に入れる。

```yaml
settings:
  grad_year: 2028     # 3年生・院1年なら「今の年度 + 2」
```

サイト定義には年度を直接書いていない（`{grad_yy}` に埋められる）ので、
**卒業年の違う友人に渡しても、YAML を書き換える必要はない**。
未設定のまま実行すると、空振りせずに何を設定すべきか名指しで出る。
...
├── oauth_client.json        「Google でログイン」用（配る人が用意して同梱する）
...
│   ├── google_setup.py      Google へのつなぎ方の判定とログイン
