...
Python を入れて `setup.bat` → `setup_gui.bat`（設定画面）の2つで済みます。
設定画面で「Google でログイン」を1回押せば、以後は全自動でカレンダーに入り、
スマホにも通知が届きます。ファイルの編集も Google Cloud の設定も要りません（15分ほど）。
そのための準備（1回だけ）が下の手順です。

以下は、仕組みまで含めて把握したい人向けの詳しい説明です。

### Google でログイン方式を用意する（配る人の作業）

友人の Google Cloud 作業をゼロにするための、**あなたが1回だけやる**手順。
できあがった `oauth_client.json` をリポジトリに含めて配れば、
受け取る側は「Google でログイン」を押すだけになる。

1. [Google Cloud Console](https://console.cloud.google.com/) でプロジェクトを作る
2. **Google Calendar API** を有効化する
3. 「OAuth 同意画面」を設定する
   - ユーザーの種類: **外部**
   - アプリ名・サポートメール・デベロッパーの連絡先を入れる
   - スコープに `https://www.googleapis.com/auth/calendar.events` を追加する
   - **公開ステータスを「本番」にする**（ここが重要。「テスト」のままだと
     リフレッシュトークンが7日で失効し、友人のツールがある日静かに止まる）
4. 「認証情報」→「認証情報を作成」→「OAuth クライアント ID」
   - アプリケーションの種類: **デスクトップ アプリ**
   - 作成したら JSON をダウンロードする
5. ダウンロードした JSON を `oauth_client.json` という名前でリポジトリ直下に置き、
   コミットして push する（`oauth_client.example.json` が雛形）

> **利用者は100人まで**。審査を通していないアプリの上限で、
> 友人数人に配る用途なら問題にならない。超えるなら Google の審査が要る。

> **client_secret をリポジトリに置くことについて**: デスクトップアプリの
> シークレットは Google 自身も機密として扱わない前提だが、公開リポジトリに
> 置けば第三者が使える。悪用されたらコンソールから無効化して作り直せばよい。
> 気になるなら `.gitignore` の該当行のコメントを外し、友人には手渡しする。

> 友人には**初回に「このアプリは確認されていません」と出ること**を
> 必ず伝えておくこと。何も知らずに見ると、まず進めない見た目をしている。
...
締切をカレンダーに入れる方法は3つある。**普通は A で十分。**

| | A. Google でログイン | B. ICS ファイル | C. サービスアカウント |
|---|---|---|---|
| 受け取る側の準備 | **ログイン1回だけ** | **なし** | Google Cloud で20分 |
| 毎日の手間 | **なし（全自動）** | 取り込み操作 数クリック | なし（全自動） |
| スマホ通知 | 届く | 届く | 届く |
| 追加インストール | `setup_google.bat` | なし | `setup_google.bat` |
| 対応カレンダー | Google のみ | Google / Outlook / Apple | Google のみ |
| 注意 | 初回に「確認されていません」の警告 | 取り込みを忘れると古いまま | 設定が重い |

**人に配るなら A。** 受け取る側の作業が「Google でログイン」を1回押すだけになる。
そのための OAuth クライアントは配る側が1回だけ用意する
（→「[Google でログイン方式を用意する（配る人の作業）](#google-でログイン方式を用意する配る人の作業)」）。

#### A. Google でログイン（推奨）

配布物に `oauth_client.json` が含まれていれば、設定画面の
**「Google でログイン」** を押すだけ。ブラウザが開くのでアカウントを選び、
